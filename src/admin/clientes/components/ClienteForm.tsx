...
import { useCrearCliente } from "../pages/CrearClientes/hooks/useCrearCliente";
import { useActualizarCliente } from "../pages/EditarClientes/hooks/useActualizarCliente";
...
interface ClienteFormProps {
  ...
  defaultValues?: ClienteFormValues & { id?: number };
}

export function ClienteForm({ onSubmit, defaultValues }: ClienteFormProps) {
  const {
    register,
    handleSubmit,
    control,
  ...
  const onFormSubmit = (data: ClienteFormValues) => {
    if (defaultValues?.id) {
      actualizarClienteMutation.mutate(
        { id: defaultValues.id, data },
        {
          onSuccess: (clienteActualizado) => {
            queryClient.setQueryData(["cliente", defaultValues.id], clienteActualizado);
            toast.success("Cliente actualizado correctamente");
            reset({ ...clienteActualizado, edad: String(clienteActualizado.edad) });
      ...
      crearClienteMutation.mutate(data, {
        ...
        onError: () => toast.error("El Correo debe ser único"),
      });
    }
  };
  ...
  useEffect(() => {
    if (defaultValues) {
      reset({ ...defaultValues, edad: String(defaultValues.edad) });
    }
  }, [defaultValues, reset]);
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{defaultValues ? "Editar Cliente" : "Crear Cliente"}</CardTitle>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-6">
          {/* Primera fila */}
          <fieldset
            disabled={actualizarClienteMutation.isPending || crearClienteMutation.isPending}
          >
          ...
          <Button
            ...
            variant={"secondary"}
          >
            {actualizarClienteMutation.isPending || crearClienteMutation.isPending
              ? "Guardando..."
              : defaultValues
              ? "Guardar cambios"
              : "Crear Cliente"}
